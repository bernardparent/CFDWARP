...
\title{
  SOAP: Scientific Oriented Algorithmic Parser 
}
...
SOAP (Scientific Oriented Algorithmic Parser) is a computer language of interpreter type, not unlike Perl, Python, or Java. SOAP is designed to be used as an interface between a C code and a control file in user space. SOAP differs from other interpreter languages by having a syntax and arithmetic that is oriented towards the engineer or physical scientist.

\subsection{Raison d'\^etre}


I needed a good user interface for my CFDWARP code. For some, a good interface might mean point-and-click buttons as part of a GUI. For me, a good interface is one that allows as much freedom in user space as possible, which generally can only be achieved through a programming language.

There are many programming languages freely available that could have been used for this project, but none proved satisfactory; they were too complex and difficult to use. Moreover, none supported arithmetic in the way I wanted; they all seemed to be designed with computer scientists in mind. I wanted my interface language to be designed with engineers or physical scientists in mind, offering predictable and user-friendly arithmetic.

A second reason for creating a specific language for CFDWARP rather than using an existing one (like Python or Perl) is portability. CFDWARP is a CFD code that is intended for use not only on desktop computers but also on supercomputers with tens of thousands of cores. When designing code expected to run on various platforms, it's better to keep the code as independent of external libraries as possible. If CFDWARP were to use Python or Perl, it would need to link to Python and Perl libraries, which may not be available on supercomputers. Additionally, libraries change over time and may offer slightly different functionalities, leading to portability issues. By not linking CFDWARP to external libraries, these portability issues are avoided hence significantly reducing the maintenance required.

Moreover, writing my own language had a third benefit: I had never written an interpreter before, and accomplishing this task helped me better understand the technicalities involved in language design.
