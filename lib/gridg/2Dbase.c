...
void JoinAll2D(GRIDG_gl2d_t gl2d, GRIDG_xygrid_t *xygrid, SOAP_codex_t *codex,
               long i1, long j1, long i2, long j2){
  long i,j;
  ...
  for (i=i1+1; i<i2; i++){
    for (j=j1+1; j<j2; j++){
      ...
      if (!xygrid[GRIDG_ai2(gl2d,i1,j)].INIT){
        SOAP_fatal_error(codex,"Node (%ld,%ld) not yet initialized. JoinAll command failed.",i1,j);
      }
      if (!xygrid[GRIDG_ai2(gl2d,i2,j)].INIT){
        SOAP_fatal_error(codex,"Node (%ld,%ld) not yet initialized. JoinAll command failed.",i2,j);
      }
      if (!xygrid[GRIDG_ai2(gl2d,i,j1)].INIT){
        SOAP_fatal_error(codex,"Node (%ld,%ld) not yet initialized. JoinAll command failed.",i,j1);
      }
      if (!xygrid[GRIDG_ai2(gl2d,i,j2)].INIT){
        SOAP_fatal_error(codex,"Node (%ld,%ld) not yet initialized. JoinAll command failed.",i,j2);
      }
  ...
  if (!VALID){
    SOAP_fatal_error(codex,"When using the JoinAll() command, set i2>i1+1 and j2>j1+1.");
  }
}
...
static void JoinAll_argum(char **argum, SOAP_codex_t *codex, GRIDG_gl2d_t gl2d, GRIDG_xygrid_t *xygrid){
  long i1,j1,i2,j2;
  int eos=EOS;
  ...
  verify_zone_validity(i1, j1, i2, j2, gl2d, codex);
  JoinAll2D(gl2d, xygrid, codex, i1, j1, i2, j2);
}
...
static void actions(char *actionname, char **argum, SOAP_codex_t *codex){
  ...
  if (strcmp(actionname,"JoinAll")==0) {
    JoinAll_argum(argum,codex,*gl2d,*xygrid);
    codex->ACTIONPROCESSED=TRUE;
  }
