...
\begin{table*}[b]
  ...
  \begin{threeparttable}
    ...
    e$^-$         & ${\rm avgh}\left( N_{\rm n}^{-1} \cdot 3.74\cdot 10^{19} \cdot {\rm exp}\left(33.5 \cdot \left({\rm ln}\, T_{\rm e} \right)^{-0.5}\right) ~,~~N_{\rm i}^{-1} \cdot 1.9 \cdot 10^{16} \cdot T_{\rm e}^{1.5}  \right)$  & \cite[Ch.\ 21]{book:1997:grigoriev}\tnote{c}\\
...
\end{equation}
%
where $e$ is the elementary charge, $N_{\rm i}$ the positive ion number density (sum of all positive ions), $m_{\rm e}$ the electron mass, and $\ln \Lambda$ varies between 5 and 8 depending on the type of ion. Setting $\ln \Lambda$ to 5, we obtain:
%
\begin{equation}
\mu_{\rm ei}= \frac{1.9 \times 10^{16}}{\rm m\cdot K^{1.5}\cdot V \cdot s} \frac{T_{\rm e}^{1.5}}{N_{\rm i}}
\end{equation}
%
