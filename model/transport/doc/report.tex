...
\end{equation}
%
where $e$ is the elementary charge, $N_{\rm i}$ the positive ion number density (sum of all positive ions), $m_{\rm e}$ the electron mass. Substituting the constants, $\mu_{\rm ei}$ becomes: 
%
\begin{equation}
\mu_{\rm ei}= \frac{9.5 \times 10^{16}}{\rm m\cdot K^{1.5}\cdot V \cdot s} \frac{T_{\rm e}^{1.5}}{N_{\rm i} \ln \Lambda}
\end{equation}
%
where $\Lambda$ is the shielding parameter and $\ln \Lambda$ is the Coulomb logarithm. The Coulomb logarithm can be  found from the NRL Plasma Formulary \cite{nrl:2013:huba}:
%
\begin{equation}
    \ln \Lambda=23-\ln\left(\sqrt{\frac{\rho_{\rm e}}{m_{\rm e}\cdot 10^6}}\cdot\left(\frac{T_{\rm e}}{11600}\right)^{-1.5}\right)
\end{equation}
%
or from Raizer's book \cite[p. 14]{book:1991:raizer}:
%
\begin{equation}
...
\subsection{Mixture Thermal Conductivity}
...
\subsection{Mass Diffusion Coefficients}

The molecular diffusion coefficient of species $k$, $\nu_k$,
is found from the binary diffusion coefficients as outlined in Dixon-Lewis \cite{gen:dixon-lewis}:
%
\begin{equation}
...
The mobility of the charged species (either electrons or ions) can be obtained from the species thermal conductivity as follows:
%
\begin{equation}
 \mu_k= \frac{\kappa_k |C_k|}{(c_p)_k \rho_k k_{\rm B} T_k}
\end{equation}
%
where $\kappa_k$ is the thermal conductivity of the $k$th species in W/m-K, $C_k$ is the charge in Coulombs of the $k$th species (eg.\ $-e$ for electrons, $+e$ for singly-charged positive ions with $e$ the elementary charge), $(c_p)_k$ is the specific heat at constant pressure of the $k$th species in J/kg-K, $\rho_k$ is the partial density of the $k$th species in kg/m$^3$, $k_{\rm B}$ the Boltzmann constant in m$^2$kg/s$^2$-K, and $T_k$ the $k$th species temperature in K.
