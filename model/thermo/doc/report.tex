...
\end{equation}
% 
Setting $\xi$ to 1.71 and $\ln \Lambda$ to 6.3, and for a single charged ion, the latter can be rewritten as:
%
\begin{equation}
\frameeqn{
...
\begin{table*}[b]
  ...
  \begin{threeparttable}
    ...
    e$^-$         & ${\rm avgharm}\left( N_{\rm n}^{-1} \cdot 3.74\cdot 10^{19} \cdot {\rm exp}\left(33.5 \cdot \left({\rm ln}\, T_{\rm e} \right)^{-0.5}\right) ~,~~N_{\rm i}^{-1} \cdot 1.061 \cdot 10^{16} \cdot T_{\rm e}^{1.5}  \right)$  & \cite[Ch.\ 21]{book:1997:grigoriev},\cite[pp.\ 180-181]{book:1984:chen}\tnote{c,d}\\
    \bottomrule
    \end{tabular*}
    \begin{tablenotes}
      ...
      \item[d] The function avgharm($a$,$b$) returns the harmonic average of the two arguments as follows $1/(a^{-1}+b^{-1})$.
    \end{tablenotes}
   \end{threeparttable}
\end{table*}
%
...
The mobilities of the various charged species are taken from Table \ref{tab:mobilities}. Note that when the ionization fraction exceeds 0.001 or so, the electron mobility needs to be corrected to account for electron-ion collisions as follows:
%
\begin{equation}
