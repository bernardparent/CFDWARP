...
void condition_Lambda_plus_minus_Parent_old(np_t *np, gl_t *gl, long lp0, long theta, jacvars_t jacvarsp0, jacvars_t jacvarsp1, metrics_t metrics,  sqmat_t Lambdaplus, sqmat_t Lambdaminus){
  ...
  Vmass=0.0;
  for (spec=0; spec<ns; spec++) Vmass+=Lambdaplus[spec][spec]*LUstarp0[spec];
  Vmax=Vmass;
  Vmin=Vmass;
  for (flux=fluxmom; flux<fluxet-1; flux++) {
    Vmax=max(Vmax,Lambdaplus[flux][flux]*LUstarp0[flux]);
    Vmin=min(Vmin,Lambdaplus[flux][flux]*LUstarp0[flux]);
  }
  lambdaadd1=max(0.0,fabs(Vmax-Vmin)-(Lambdaplus[fluxet][fluxet]*LUstarp0[fluxet]+Lambdaplus[fluxet-1][fluxet-1]*LUstarp0[fluxet-1]))/LUstarp0[fluxet];

  Vmass=0.0;
  for (spec=0; spec<ns; spec++) Vmass+=-Lambdaminus[spec][spec]*LUstarp1[spec];
  Vmax=Vmass;
  Vmin=Vmass;
  for (flux=fluxmom; flux<fluxet-1; flux++) {
    Vmax=max(Vmax,-Lambdaminus[flux][flux]*LUstarp1[flux]);
    Vmin=min(Vmin,-Lambdaminus[flux][flux]*LUstarp1[flux]);
  }
  lambdaadd2=max(0.0,fabs(Vmax-Vmin)-fabs(Lambdaminus[fluxet][fluxet]*LUstarp1[fluxet]+Lambdaminus[fluxet-1][fluxet-1]*LUstarp1[fluxet-1]))/LUstarp1[fluxet];

  lambdaadd=max(lambdaadd1,lambdaadd2);

  for (flux=fluxet-1; flux<=fluxet; flux++){
    Lambdaplus[flux][flux]+=0.5*lambdaadd;
    Lambdaminus[flux][flux]-=0.5*lambdaadd;
  }
}
...
void condition_Lambda_plus_minus_Parent_veryold(np_t *np, gl_t *gl, long lp0, long theta, jacvars_t jacvarsp0, jacvars_t jacvarsp1, metrics_t metrics,  sqmat_t Lambdaplus, sqmat_t Lambdaminus){
  ...
  Vmin=1e99;
  Vmax=-1e99;
  for (flux=0; flux<fluxet-1; flux++) Vmin=min(Vmin,Lambdaplus[flux][flux]*LUstarp0[flux]);
  for (flux=0; flux<fluxet-1; flux++) Vmax=max(Vmax,Lambdaplus[flux][flux]*LUstarp0[flux]);
  Vadd1=max(0.0,fabs(Vmax-Vmin)-(Lambdaplus[fluxet][fluxet]*LUstarp0[fluxet]+Lambdaplus[fluxet-1][fluxet-1]*LUstarp0[fluxet-1]))/LUstarp0[fluxet];

  Vmin=1e99;
  Vmax=-1e99;
  for (flux=0; flux<fluxet-1; flux++) Vmin=min(Vmin,-Lambdaminus[flux][flux]*LUstarp1[flux]);
  for (flux=0; flux<fluxet-1; flux++) Vmax=max(Vmax,-Lambdaminus[flux][flux]*LUstarp1[flux]);
  Vadd2=max(0.0,fabs(Vmax-Vmin)-fabs(Lambdaminus[fluxet][fluxet]*LUstarp1[fluxet]+Lambdaminus[fluxet-1][fluxet-1]*LUstarp1[fluxet-1]))/LUstarp1[fluxet];
