...
\nomenclature{
  \begin{nomenclaturelist}{Roman symbols}
   ...
   \item[$Q_{\rm J}^{\rm e}$] electron Joule heating, $\frac{1}{\mu_{\rm e}}|C_{\rm e}|N_{\rm e}|\vec{V}^{\rm e}-\vec{V}|^2$
   ...
   \item[$\vec{V}$] velocity of the neutrals   
...
%
\begin{equation}
  \vec{V}^{k}_i = \vec{V}_i+\sum_{j=1}^3 s_k \wtilde{\mu}^k_{ij}  \vec{E}_j^{\rm n}
...
\begin{equation}
\frameeqn{
  \rho_{\rm n} \frac{\partial \vec{V}_i }{\partial t}+ \sum_{j=1}^3 \rho_{\rm n} \vec{V}_j \frac{\partial \vec{V}_i}{\partial x_j}
=
...
%
\begin{equation}
\tau_{ji} = \visc  \left( \frac{\partial \vec{V}_i}{\partial x_j} + \frac{\partial \vec{V}_j}{\partial x_i} - \frac{2}{3} \delta_{ij} \sum_{k=1}^3 \frac{\partial \vec{V}_k}{\partial x_k} \right)
...
%
\begin{equation}
\rho_k \vec{V}^k_i=\rho_k \vec{V}_i - \nu_k \frac{\partial w_k}{\partial x_i}
\end{equation}
%  
where $\nu_k$ is the mass diffusion coefficient and $w_k$ the mass fraction and $\vec{V}$ the velocity of the bulk of the neutrals. Thus, the mass conservation equation of the $k$th neutral species becomes:
%
\begin{equation}
\frameeqn{
  \frac{\partial \rho_k}{\partial t} + \sum_{i=1}^3 \frac{\partial}{\partial x_i} \vec{V}_i \rho_k 
...
+ \sum_{i=1}^3 \sum_{r=1}^\ns  \frac{\partial}{\partial x_i}      \alpha_{kr} \left( \Delta \vec{V}_i^r + \vec{V}_i \right)\rho_r  
...
%
\begin{equation}
\frac{\partial \rho_k}{\partial t} + \sum_{j=1}^3 \frac{\partial}{\partial x_j} \rho_k \vec{V}_j = W_k
\end{equation}
%
Sum over all neutral species:
%
\begin{equation}
\frac{\partial \rho_{\rm n}}{\partial t} + \sum_{j=1}^3 \frac{\partial}{\partial x_j} \rho_{\rm n} \vec{V}_j = \sum_{k=1}^\nns W_k
\end{equation}
%
Multiply by $\vec{V}_i$:
%
\begin{equation}
\vec{V}_i \frac{\partial \rho_{\rm n}}{\partial t} + \sum_{j=1}^3 \vec{V}_i \frac{\partial}{\partial x_j} \rho_{\rm n} \vec{V}_j = \vec{V}_i \sum_{k=1}^\nns  W_k
\end{equation}
%
Add the latter to Eq.\ (\ref{eqn:momneutrals1}):
%
\begin{equation}
   \frac{\partial  }{\partial t}\rho_{\rm n} \vec{V}_i
  + \sum_{j=1}^3  \frac{\partial }{\partial x_j}\rho_{\rm n} \vec{V}_j \vec{V}_i
=
-\frac{\partial P_{\rm n}}{\partial x_i} 
+ \sum_{j=1}^3 \frac{\partial \tau_{ji}}{\partial x_j}
+ \sum_{r=1}^\ncs \vec{F}_i^r
...
\begin{equation}
  ...
  + \sum_{j=1}^3  \frac{\partial }{\partial x_j}\rho_{\rm n} \vec{V}_j \vec{V}_i
\approx
   \frac{\partial  }{\partial t}\rho \vec{V}_i
  + \sum_{j=1}^3  \frac{\partial }{\partial x_j}\rho \vec{V}_j \vec{V}_i  
\label{eqn:momtotal2}
\end{equation}
%    
...
- \underbrace{\left(\rho_{\rm c} \vec{E} + \vec{J}\times\vec{B} -\sum_{r=1}^\ncs \vec{\nabla} P_r \right)\cdot \vec{V}}_{\textrm{\begin{minipage}{4cm}\flushleft \footnotesize work done by the charged species on the neutrals in the lab frame\end{minipage}}}
...
%
\begin{equation}
  \rho_{\rm N_2} \frac{d \ev} {d t}  = \frac{\delta Q}{\mathrlap{-}V \Delta t} - \frac{\ev}{\mathrlap{-}V}  \frac{d M_{\rm N_2} }{d t} 
\end{equation}
%
...
%
\begin{equation}
\frac{\delta Q}{\mathrlap{-}V \Delta t} 
=
\underbrace{
...
%
\begin{equation}
 \ev W_{\rm N_2} = \frac{\ev}{\mathrlap{-}V} \frac{d M_{\rm N_2}}{dt}
\end{equation}
%
Thus:
%
...
%
\begin{equation}
 \sum_{k=1}^\nns \frac{d }{d t}\left(M_k h_k\right) - \mathrlap{-}V \frac{d P_{\rm n}}{dt} = \frac{\delta Q}{\Delta t}-\frac{\delta W}{\Delta t}
\end{equation}
%
Split the first derivative:
%
\begin{equation}
 \sum_{k=1}^\nns M_k\frac{d h_k}{d t} - \mathrlap{-}V \frac{d P_{\rm n}}{dt} = \frac{\delta Q}{\Delta t}-\frac{\delta W}{\Delta t}-\sum_{k=1}^\nns h_k\frac{d M_k}{d t}
\end{equation}
%
...
\sum_{k=1}^\nns \rho_k \frac{d h_k}{d t} -  \frac{d P_{\rm n}}{dt} = 
 \frac{\delta Q}{\mathrlap{-}V \Delta t}
-\frac{\delta W}{\mathrlap{-}V \Delta t}
-\sum_{k=1}^\nns \frac{h_k}{\mathrlap{-}V}\frac{d M_k}{d t}
...
\begin{equation}
 \frac{\delta W}{\mathrlap{-}V \Delta t}
=
- \sum_{j=1}^3 \sum_{i=1}^3 \tau_{ji} \frac{\partial \vec{V}_i}{\partial x_j} 
- \sum_{r=1}^\ncs \sum_{i=1}^3 \vec{F}_i^{r} \left( \vec{V}_i^{{\rm n}-r}-\vec{V}_i \right)
\end{equation}
%
where $\tau_{ji}$ is the shear stress of the neutrals and $\vec{V}_j$ is the bulk velocity of the neutrals including drift and diffusion. As well $\vec{F}_i^r$ is the collision force by the charged species $r$ on the neutrals and $\vec{V}_i^{{\rm n}-r}$ is the average velocity at which the collisions between the charged species $r$ and the neutrals take place.  The last term on the RHS of Eq.\ (\ref{eqn:eneutrals2}) can also be written as:
%
\begin{equation}
  \frac{h_k}{\mathrlap{-}V}\frac{d M_k}{d t}
=
  h_k W_k
\end{equation}
%
...
%
\begin{equation}
\frac{d P_{\rm n}}{d t} = \frac{\partial P_{\rm n}}{\partial t} + \sum_{i=1}^3 \vec{V}_i \frac{\partial P_{\rm n}}{\partial x_i}
\end{equation}
%
...
- \frac{\partial P_{\rm n}}{\partial t} - \sum_{i=1}^3 \vec{V}_i \frac{\partial P_{\rm n}}{\partial x_i}
= \alb\mfd
-\sum_{i=1}^{3}\frac{\partial q_i}{\partial x_i}
+\sum_{r=1}^{\ncs} Q_{r}
+\sum_{i=1}^3 \sum_{j=1}^3 \tau_{ji} \frac{\partial \vec{V}_i}{\partial x_j}
+ \sum_{r=1}^\ncs \sum_{i=1}^3 \vec{F}_i^{r} \left( \vec{V}_i^{{\rm n}-r}-\vec{V}_i \right) \alb\mfd
- \sum_{k=1}^\nns h_k W_k 
- \sum_{k=1}^\nns h_k^\circ W_k
-  Q_{\rm v}
+ Q_{\rm b}
...
- \sum_{i=1}^3 \vec{V}_i \frac{\partial P_{\rm n}}{\partial x_i}
=\alb\mfd 
-\sum_{i=1}^{3}\frac{\partial q_i}{\partial x_i}
+\sum_{r=1}^{\ncs} Q_{r}
+\sum_{i=1}^3 \sum_{j=1}^3 \tau_{ji} \frac{\partial \vec{V}_i}{\partial x_j}
...
\end{equation}
%
Multiply the latter by $\vec{V}_i$ and sum over all $i$s:
%
\begin{equation}
  \sum_{i=1}^3 \rho_{\rm n} \vec{V}_i \frac{\partial \vec{V}_i }{\partial t}
+ \sum_{i=1}^3 \sum_{j=1}^3 \rho_{\rm n} \vec{V}_j \vec{V}_i \frac{\partial \vec{V}_i}{\partial x_j}
+ \sum_{i=1}^3 \vec{V}_i \frac{\partial P_{\rm n}}{\partial x_i} 
=
+ \sum_{i=1}^3 \sum_{j=1}^3 \vec{V}_i \frac{\partial \tau_{ji}}{\partial x_j}
+ \sum_{i=1}^3 \sum_{r=1}^\ncs \vec{F}_i^r \vec{V}_i
\end{equation}
%
Rearrange:
%
\begin{equation}
   \rho_{\rm n}  \frac{\partial  }{\partial t}\left(\frac{1}{2}|\vec{V}|^2\right)
+  \sum_{j=1}^3 \rho_{\rm n} \vec{V}_j  \frac{\partial }{\partial x_j}\left(\frac{1}{2}|\vec{V}|^2\right)
+ \sum_{i=1}^3 \vec{V}_i \frac{\partial P_{\rm n}}{\partial x_i} 
=
+ \sum_{i=1}^3 \sum_{j=1}^3 \vec{V}_i \frac{\partial \tau_{ji}}{\partial x_j}
+ \sum_{i=1}^3 \sum_{r=1}^\ncs \vec{F}_i^r \vec{V}_i
...
%
\begin{equation}
 \frac{\partial }{\partial t}\rho_{\rm n} + \sum_{j=1}^3 \frac{\partial }{\partial x_j} \rho_{\rm n} \vec{V}_j = \sum_{k=1}^\nns W_k
\end{equation}
%
Multiply by $\frac{1}{2}|\vec{V}|^2$:
%
\begin{equation}
 \frac{1}{2}|\vec{V}|^2\frac{\partial }{\partial t}\rho_{\rm n} 
+ \frac{1}{2}|\vec{V}|^2 \sum_{j=1}^3 \frac{\partial }{\partial x_j} \rho_{\rm n} \vec{V}_j 
= \frac{1}{2}|\vec{V}|^2 \sum_{k=1}^\nns W_k
\end{equation}
%
...
\begin{equation}
\begin{array}{l}\mfd
     \frac{\partial  }{\partial t}\left( \frac{1}{2}\rho_{\rm n}|\vec{V}|^2\right)
+  \sum_{j=1}^3   \frac{\partial }{\partial x_j}\left(\frac{1}{2}\rho_{\rm n} \vec{V}_j |\vec{V}|^2\right)
+ \sum_{i=1}^3 \vec{V}_i \frac{\partial P_{\rm n}}{\partial x_i} 
\alb\mfd
=
+ \sum_{i=1}^3 \sum_{j=1}^3 \vec{V}_i \frac{\partial \tau_{ji}}{\partial x_j}
...
\frameeqn{
\begin{array}{l}\mfd
 \frac{\partial }{\partial t}\left(\sum_{k=1}^\nns \rho_k (e_k+h_k^\circ)+\frac{1}{2}\rho_{\rm n}|\vec{V}|^2 \right) 
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(\sum_{k=1}^\nns \rho_k \vec{V}^k_j (h_k+h_k^\circ)+\frac{1}{2}\rho_{\rm n}\vec{V}_j|\vec{V}|^2 \right)\alb\mfd
= 
-\sum_{i=1}^{3}\frac{\partial q_i}{\partial x_i}
+\sum_{r=1}^{\ncs} Q_{r}
+\sum_{i=1}^3 \sum_{j=1}^3  \frac{\partial }{\partial x_j} \tau_{ji} \vec{V}_i
...
%
\begin{equation}
 \frac{d }{d t}\left(M_k h_k\right) - \mathrlap{-}V \frac{d P_k}{dt} = \frac{\delta Q}{\Delta t}-\frac{\delta W}{\Delta t}
\end{equation}
%
Split the first derivative:
%
\begin{equation}
 M_k\frac{d h_k}{d t} - \mathrlap{-}V \frac{d P_k}{dt} = \frac{\delta Q}{\Delta t}-\frac{\delta W}{\Delta t}-h_k\frac{d m_k}{d t}
\end{equation}
%
...
%
\begin{equation}
\rho_k \frac{d h_k}{d t} -  \frac{d P_k}{dt} = \frac{\delta Q}{\mathrlap{-}V \Delta t}-\frac{\delta W}{\mathrlap{-}V \Delta t}
-\frac{h_k}{\mathrlap{-}V}\frac{d M_k}{d t}
\label{eqn:echarged2}
\end{equation}
%  
...
\frac{\delta Q}{\mathrlap{-}V \Delta t}= 
\underbrace{
-\sum_{j=1}^3 \frac{\partial q^{k}_j}{\partial x_j}
}_{\begin{minipage}{1.7cm}\flushleft \footnotesize conduction heat transfer to surroundings\end{minipage}}
-
\underbrace{
Q_k
...
%
\begin{equation}
\frac{\delta W}{\mathrlap{-}V \Delta t}= \sum_{i=1}^3 \vec{F}_i^k \left( \vec{V}_i^{{\rm n}-k}-\vec{V}_i^k \right)
\end{equation}
%
...
%
\begin{equation}
  \frac{h_k}{\mathrlap{-}V}\frac{d M_k}{d t} = h_k W_k
\end{equation}
%
...
- \sum_{i=1}^3 \vec{V}^k_i \frac{\partial P_k}{\partial x_i}
\alb\mfd= -\sum_{j=1}^3 \frac{\partial q^{k}_j}{\partial x_j}
- Q_{k}
-\sum_{i=1}^3 \vec{F}_i^k \left( \vec{V}_i^{{\rm n}-k}-\vec{V}_i^k \right)
\end{array}
\label{eqn:echarged4}
...
%
\begin{equation}
  \vec{V}^{k}=\vec{V} - \mu_{k} \vec{E}^{k} -\frac{\mu_{k}}{|C_{k}| N_{k}}\nabla P_{k}
\end{equation}
% 
or,
%
\begin{equation}
  \vec{V}^{k}_j=\vec{V}_j - \mu_{\rm e} \vec{E}^{k}_j -\frac{\mu_{k}}{|C_{k}| N_{k}} \frac{\partial P_{k}}{\partial x_j} 
\end{equation}
% 
Expand the pressure gradient term:
%
\begin{equation}
  \vec{V}^{k}_j=\vec{V}_j - \mu_{k} \vec{E}^{k}_j 
...
\begin{equation}
  ...
  = \rho_{k} (h_{k}+h_k^\circ)\left( \vec{V}_j 
...
Finally, the heat flux can be written as:
%
\begin{equation}
q^k_j = -\kappa_k \frac{\partial T_k}{\partial x_j}
\end{equation}
%
with the $k$th charged species thermal conductivity obtained from either the Gupta-Yos model or the Dixon-Lewis model. In the case of the latter,
%
\begin{equation}
  \kappa_k = \frac{\rho_{k} {h}_{k} \mu_{k} k_{\rm B}}{|C_{k}| }
\end{equation}
%
...
%
\begin{equation}
 \frac{d }{d t}\left(M_{\rm e} h_{\rm e}\right) - \mathrlap{-}V \frac{d P_{\rm e}}{dt} = \frac{\delta Q}{\Delta t}-\frac{\delta W}{\Delta t}
\end{equation}
%
...
%
\begin{equation}
 M_{\rm e}\frac{d h_{\rm e}}{d t} - \mathrlap{-}V \frac{d P_{\rm e}}{dt} = \frac{\delta Q}{\Delta t}-\frac{\delta W}{\Delta t}-h_{\rm e}\frac{d M_{\rm e}}{d t}
\end{equation}
%
Divide through by the volume of the system $\mathrlap{-}V$:
%
\begin{equation}
\rho_{\rm e} \frac{d h_{\rm e}}{d t} -  \frac{d P_{\rm e}}{dt} = \frac{\delta Q}{\mathrlap{-}V \Delta t}-\frac{\delta W}{\mathrlap{-}V \Delta t}
-\frac{h_{\rm e}}{\mathrlap{-}V}\frac{d M_{\rm e}}{d t}
...
\frac{\delta Q}{\mathrlap{-}V \Delta t}= 
\underbrace{
-\sum_{j=1}^3 \frac{\partial q^{\rm e}_j}{\partial x_j}
}_{\begin{minipage}{1.7cm}\flushleft \footnotesize conduction heat transfer to surroundings\end{minipage}}
+
\underbrace{
W_{\rm e} e_{\rm e}
}_{\begin{minipage}{1.7cm}\flushleft \footnotesize heat gain/lost  due to creation/destruction of electrons\end{minipage}}
-
\underbrace{
Q_{\rm ic}
}_{\begin{minipage}{1.7cm}\flushleft \footnotesize heat lost due to inelastic collisions \end{minipage}}
\end{equation}
%
...
%
\begin{equation}
\frac{\delta W}{\mathrlap{-}V \Delta t}= \sum_{i=1}^3 \vec{F}_i^{\rm e} \left( \vec{V}_i^{{\rm n}-{\rm e}}-\vec{V}_i^{\rm e} \right)
\end{equation}
%
...
%
\begin{equation}
  \frac{h_{\rm e}}{\mathrlap{-}V}\frac{d M_{\rm e}}{d t} = h_{\rm e} W_{\rm e}
\end{equation}
%
Also change the frame of reference from the species frame to the lab frame:
%
...
\end{equation}
%  
We can then define  $Q_{\rm e}$ as:
%
\begin{equation}
Q_{\rm e} \equiv
\underbrace{Q_{\rm ic}}_{\begin{minipage}{3cm} \footnotesize \flushleft energy lost to neutrals due to inelastic collisions\end{minipage}} + \underbrace{\vec{F}^{\rm e}\cdot  \vec{V}^{{\rm n}-{\rm e}}}_{\begin{minipage}{3cm} \footnotesize \flushleft energy lost to neutrals due to elastic collisions\end{minipage}} 
\end{equation}
%
Thus:
%
...
%
\begin{equation}
  \vec{V}^{\rm e}=\vec{V} - \mu_{\rm e} \vec{E}^{\rm e} -\frac{\mu_{\rm e}}{|C_{\rm e}| N_{\rm e}}\nabla P_{\rm e}
\end{equation}
% 
or,
%
\begin{equation}
  \vec{V}^{\rm e}_j=\vec{V}_j - \mu_{\rm e} \vec{E}^{\rm e}_j -\frac{\mu_{\rm e}}{|C_{\rm e}| N_{\rm e}} \frac{\partial P_{\rm e}}{\partial x_j} 
\end{equation}
% 
Expand the electron pressure:
%
\begin{equation}
  \vec{V}^{\rm e}_j=\vec{V}_j - \mu_{\rm e} \vec{E}^{\rm e}_j 
...
\end{equation}
% 
Add the electron heat diffusion on both sides:
%
\begin{equation}
  \rho_{\rm e} h_{\rm e} \vec{V}^{\rm e}_j -\kappa_{\rm e} \frac{\partial T_{\rm e}}{\partial x_j}
  = \rho_{\rm e} h_{\rm e} \vec{V}_j 
  - \rho_{\rm e} h_{\rm e} \mu_{\rm e} \vec{E}^{\rm e}_j 
  -\frac{\rho_{\rm e} h_{\rm e} \mu_{\rm e} k_{\rm B}}{|C_{\rm e}| } \frac{  \partial T_{\rm e}}{\partial x_j} 
  -\frac{\rho_{\rm e} h_{\rm e} \mu_{\rm e} k_{\rm B} T_{\rm e}}{|C_{\rm e}| N_{\rm e}} \frac{\partial N_{\rm e}  }{\partial x_j} 
  -\kappa_{\rm e} \frac{\partial T_{\rm e}}{\partial x_j}
\end{equation}
% 
...
%
\begin{equation}
  \rho_{\rm e} h_{\rm e} \left( \vec{V}^{\rm e}_j - \vec{V}_j \right)
  = 
  - \rho_{\rm e} h_{\rm e} \mu_{\rm e} \left( \vec{E}+(\vec{V}^{\rm e}-\vec{V})\times\vec{B}\right)_j 
  -\frac{5 N_{\rm e} k_{\rm B}^2 T_{\rm e} \mu_{\rm e} }{2 |C_{\rm e}| } \frac{  \partial T_{\rm e}}{\partial x_j} 
  -\frac{5 k_{\rm B}^2 T_{\rm e}^2 \mu_{\rm e}}{2 |C_{\rm e}| } \frac{\partial N_{\rm e}  }{\partial x_j} 
  - \rho_{\rm e} h_{\rm e} \mu_{\rm e} \left( \vec{V}\times\vec{B}\right)_j 
\end{equation}
% 
Rearrange again:
%
\begin{equation}
  \rho_{\rm e} h_{\rm e} \left( \vec{V}^{\rm e} - \vec{V} -s_{\rm e} \mu_{\rm e}(\vec{V}^{\rm e}-\vec{V})\times\vec{B}\right)_j
  = 
  - \rho_{\rm e} h_{\rm e} \mu_{\rm e} \left( \vec{E}+ \vec{V}\times\vec{B}\right)_j 
  -\frac{5 N_{\rm e} k_{\rm B}^2 T_{\rm e} \mu_{\rm e} }{2 |C_{\rm e}| } \frac{  \partial T_{\rm e}}{\partial x_j} 
  -\frac{5 k_{\rm B}^2 T_{\rm e}^2 \mu_{\rm e}}{2 |C_{\rm e}| } \frac{\partial N_{\rm e}  }{\partial x_j} 
\end{equation}
% 
...
%
\begin{equation}
  \rho_{\rm e} h_{\rm e} \left( \vec{V}^{\rm e} - \vec{V} \right)_i
  = 
  - \sum_j \rho_{\rm e} h_{\rm e} \tilde{\mu}_{ij}^{\rm e} \left( \vec{E}+ \vec{V}\times\vec{B}\right)_j
...
= -\sum_{i=1}^3 \frac{\partial q^{\rm e}_i}{\partial x_i}
+ W_{\rm e} e_{\rm e}
+   C_{\rm e} N_{\rm e} \vec{E} \cdot \vec{V}^{\rm e}  
- Q_{\rm e}
\end{equation}
...
\frameeqn{
\begin{array}{l}\mfd
 \frac{\partial }{\partial t}\rho_{\rm e} e_{\rm e} + \sum_{i=1}^3  \frac{\partial }{\partial x_i} \rho_{\rm e} \vec{V}_i h_{\rm e} 
  - \sum_{i=1}^3 \sum_{j=1}^3 \frac{\partial}{\partial x_i} \rho_{\rm e} h_{\rm e} \tilde{\mu}_{ij}^{\rm e} (\vec{E}+\vec{V}\times\vec{B})_j 
...
\end{equation}
%  
In the latter, the electron heat flux in the presence of a magnetic field corresponds to \cite{book:2002:huba}:
%
\begin{equation}
q^{\rm e}_i = -\sum_{j=1}^3  \frac{5 N_{\rm e} k_{\rm B}^2 T_{\rm e} \tilde{\mu}_{ij}^{\rm e} }{2 |C_{\rm e}| } \frac{  \partial T_{\rm e}}{\partial x_j}  
\end{equation}
%
In the absence of a magnetic field, the electron thermal conductivity simplifies to \cite{ht:1997:raizer}:
%
\begin{equation}
\kappa_{\rm e}=\frac{5 k_{\rm B}^2 N_{\rm e} T_{\rm e} \mu_{\rm e}}{2|C_{\rm e}|}
\end{equation}
%
...
\begin{table}
  ...
  \begin{threeparttable}
    ...
    \toprule
    \multicolumn{2}{c}{$|\vec{E}+\vec{V}^{\rm e} \times \vec{B}|/N$ [V~m$^2$]}  & \multicolumn{2}{c}{$T_{\rm e}$ [eV]}   &  \multicolumn{2}{c}{$T_{\rm e}$ [K]} \\
    \midrule
...
\begin{table}
  ...
  \begin{threeparttable}
    ...
    \toprule
    \multicolumn{2}{c}{$|\vec{E}+\vec{V}^{\rm e} \times \vec{B}|/N$ [V~m$^2$]}  & \multicolumn{2}{c}{$(T_{\rm e})_{\rm N_2}$ [eV]} & \multicolumn{2}{c}{$(T_{\rm e})_{\rm O_2}$ [eV]} & \multicolumn{2}{c}{$T_{\rm e}$ [eV]} &  \multicolumn{2}{c}{$T_{\rm e}$ [K]} \\
    \midrule
...
\vec{E}^k \cdot \vec{V}^k 
 = \vec{E}^k \cdot \left( \vec{V} + s_k \mu_k \vec{E}^k-\frac{\mu_k}{|C_k| N_k}\nabla P_k \right)
\end{equation}
% 
...
\vec{E}^k \cdot \vec{J}^k 
 = C_k N_k \vec{E}^k \cdot \left( \vec{V} + s_k \mu_k \vec{E}^k-\frac{\mu_k}{|C_k| N_k}\nabla P_k \right)
\end{equation}
% 
Rearrange the RHS:
%
\begin{equation}
\vec{E}^k \cdot \vec{J}^k 
 = C_k N_k \vec{E}^k \cdot \left( \vec{V} -\frac{\mu_k}{|C_k| N_k}\nabla P_k \right)
...
\frameeqn{
\vec{E}^k \cdot \vec{J}^k 
 =   C_k N_k \vec{E}^k \cdot \vec{V} - s_k \mu_k \vec{E}^k \cdot \nabla P_k 
  +|C_k| \mu_k N_k |\vec{E}^k|^2
}
\end{equation}
% 
...
\subsection{Electron Energy Loss Function}
...
\end{equation}
%
Where $Q_{\rm ei}$ is the electron energy loss due to electron impact and $Q_{\rm e-n}$ is the electron energy loss due to elastic collisions:
%
\begin{equation}
Q_{\rm e-n} \equiv \vec{F}^{\rm e}\cdot  \vec{V}^{{\rm n}-{\rm e}} 
\end{equation}
%
The collision force between the electrons and the neutrals corresponds to (see momentum equation for the charged species):
%
\begin{equation}
\vec{F}^{\rm e} = \frac{|C_{\rm e}|N_{\rm e}}{\mu_{\rm e}} \left(\vec{V}^{\rm e} - \vec{V} \right)
\end{equation}
%
Thus:
%
\begin{equation}
Q_{\rm e-n} = \frac{|C_{\rm e}|N_{\rm e}}{\mu_{\rm e}} \left(\vec{V}^{\rm e} - \vec{V} \right) \cdot  \vec{V}^{{\rm n}-{\rm e}} 
\end{equation}
%
further assume that either the electron velocity is much greater than the neutrals velocity and that the velocity of the collision $\vec{V}^{\rm e}$ is approximately the electron velocity. Then, we obtain:
%
\begin{equation}
...
\end{equation}
%
Substitute in $Q_{\rm e-n}$ expression:
%
\begin{equation}
...
%
\begin{equation}
  \vec{V}^{\rm e}=\vec{V} - \mu_{\rm e} \left(\vec{E}+\vec{V}^{\rm e} \times \vec{B}\right)
\end{equation}
% 
...
=  
 \frac{2 m_{\rm e} \mu_{\rm e}}{3 |C_{\rm e}| N_{\rm e} k_{\rm B} T_{\rm e} }
\left( C_{\rm e} N_{\rm e} \left(\vec{E} + \vec{V^{\rm e}} \times \vec{B}\right)\cdot \left(\vec{V} - \mu_{\rm e} \left(\vec{E}+\vec{V}^{\rm e} \times \vec{B}\right)\right)
 - Q_{\rm ei}\right)
 \end{equation}
%
...
\subsection{Parent 2021 Electron Energy Relaxation Model}

In this subsection, we will derive the Parent 2021 relaxation source terms of the electron energy transport equation presented in \cite{pof:2021:parent}.  
...
\zeta_{\rm e} 
=
 \frac{2}{3} \frac{m_{\rm e} \left(\mu_{\rm e} N\right)}{k_{\rm B} (T_{\rm e}^\prime-T)} 
\left[  
  \left(\mu_{\rm e} N\right)  \left( \frac{|\vec{E}+\vec{V}^{\rm e}\times \vec{B}|}{N}\right)^2
 -15.65~\frac{\rm J}{\rm C}\cdot \left( \frac{N_{\rm N_2}}{N}\right)  k_{\rm ei}^{\rm N_2} \right.
\alb\mfd
 \left. -12.15~\frac{\rm J}{\rm C}\cdot \left( \frac{N_{\rm O_2}}{N}\right) k_{\rm ei}^{\rm O_2}
 \right]
\end{array}
 \end{equation}
%
But:
\begin{equation}
|\vec{E}+\vec{V}^{\rm e} \times \vec{B}|/N=E^\star
\end{equation}
%
Thus:
%
...
\left[  
\begin{array}{r}\mfd
  \left(\mu_{\rm e} N\right)  \left(E^\star\right)^2
 -15.65~\frac{\rm J}{\rm C}\cdot \left( \frac{N_{\rm N_2}}{N}\right)  k_{\rm ei}^{\rm N_2}
 -12.15~\frac{\rm J}{\rm C}\cdot \left( \frac{N_{\rm O_2}}{N}\right) k_{\rm ei}^{\rm O_2}
 \end{array}
 \right]
 \label{eqn:oldzetae2}
 \end{equation}
%
In the latter, $\mu_{\rm e} N$ and $T_{\rm e}^\prime$ can be expressed as a function of the reduced electric field for a given reference neutrals temperature $T$ (here set to 250~K). As well, for air in which the mass fractions of molecular oxygen and nitrogen are of 0.235 and 0.765 respectively, we get ${N_{\rm N_2}}/{N}=0.788$ and ${N_{\rm O_2}}/{N}=0.212$. 
...
\subsection{Parent 2024 Electron Energy Relaxation Model}


Let us start from the electron energy transport equation for a weakly-ionized plasma composed of one neutral species, one ion species, and electrons:
%
\begin{equation}
\frac{\partial }{\partial t}\rho_{\rm e} e_{\rm e} + \sum_{j=1}^3  \frac{\partial }{\partial x_j} \rho_{\rm e} \vec{V}^{\rm e}_j h_{\rm e} 
= 
...
\end{equation}
%
where $Q_{\rm e-n}$ contains all electron-impact energy losses and relaxation processes with neutrals (elastic and inelastic processes). Consider the energy transport equation in the ``local approximation'', where the plasma has uniform properties (with no spatial gradients) and is at steady-state. Then, the latter becomes:
%
\begin{equation}
0
= 
...
Q_{\rm e-n}
= 
   C_{\rm e} N_{\rm e} (\vec{E}+\vec{V}^{\rm e}\times\vec{B}) \cdot \left( \vec{V} - \mu_{\rm e} \left(\vec{E}+\vec{V}^{\rm e} \times \vec{B}\right) \right)
\end{equation}
%
...
\end{equation}
%
The latter is valid when the plasma is composed of one dominant neutral species, electrons, and one ion species. For a weakly-ionized plasma with multiple neutral species, the latter becomes:
%
\begin{equation}
\frameeqn{
Q_{\rm e-n}
= 
  \sum_k  \beta_k^{\rm n} |C_{\rm e}| N_{\rm e} N_k (\mu_{\rm e}  N)_k  (E^\star_k)^2
}
\label{eqn:Qe-n}
\end{equation}
%
...
The above expression for $Q_{\rm e-n}$ will predict accurately all electron losses with neutrals including electron impact ionization and excitation, elastic collisions, and inelastic collisions with the vibrational energy modes of the molecule. However, it does suffer from not predicting electron energy gains due to collisions with neutrals when the electron temperature is less than the heavy particules temperatures. It also suffers from not predicting electron energy gains and losses with the ions.

The electron energy gains/losses with all heavy particules (either neutrals or ions) due to elastic collisions can be obtained following \cite[page 16]{book:1991:raizer} and
...
The energy transfer rate of excitation of molecular vibrations due to collisions of molecules with electrons can be obtained following \cite{jap:2010:shneider}. 
%
\begin{equation}
\frameeqn{
Q_{\rm e-v} = \sum_k \frac{3}{2} N_{\rm e} k_{\rm B} (T_{\rm e}-T_{\rm v}) \nu_{{\rm ev},k} 
}
\label{eqn:Qe-v}
\end{equation}
%
with $\nu_{\rm ev}$ the electron-vibration excitation rate in units of 1/s:
%
\begin{equation}
\nu_{{\rm ev},k} = \frac{8.917 \cdot 10^{10} \frac{N_k}{N_0} T_{\rm e}^{-3} \left(9.93 +T_{\rm e}^5\right)\exp\left(-2.36 T_{\rm e}^{-3}\right) - \nu_{{\rm ex},k} I^\star_k}{(\hbar \omega_0)_k}
\label{eqn:nuev}
\end{equation}
%
where $\nu_{\rm ex}$ the rate of electronic excitation in units of 1/s:
%
\begin{equation}
\nu_{{\rm ex},k}= 5.53 \cdot 10^{6} \frac{N_k}{N_0} T_{\rm e}^2 \left(1812.23 + T_{\rm e}^{-15} \right)\cdot \exp\left(-\frac{1.7835}{T_{\rm e}^2}\right)
\label{eqn:nuex}
\end{equation}
%
In the latter, $I^\star_k$ is the relevant effective excitation energy which is set to 10~eV for the N2 molecule \cite{tp:2000:ivanovskii}, $\hbar\omega_0$ is set to 0.29~eV for N2 molecules, $N_0$ is the gas number density at a pressure of 1 atm and a temperature of 300~K ($2.446\cdot 10^{25} \rm m^{-3}$), $N_k$ is the number density of species $k$ in units of $\rm m^{-3}$. In Eqs.\ (\ref{eqn:nuev})-(\ref{eqn:nuex}), $T_{\rm e}$ is in units of eV to obtain excitation rates of 1/s. 
 
The total electron energy loss is not simply $Q_{\rm e-t}+Q_{\rm e-v}+Q_{\rm e-n}$ as obtained from Eqs.\ (\ref{eqn:Qe-t}), (\ref{eqn:Qe-v}), and (\ref{eqn:Qe-n}) because $Q_{\rm e-n}$ includes the loss of electron energy due to elastic collisions with the neutrals and includes the loss of electron energy due to electron-vibration collisions. We thus need to subtract the loss of electron energy due to elastic collisions and due to electron-vibration from $Q_{\rm e-n}$. This can be done by finding the fraction of $E/N$ within  $Q_{\rm e-n}$ that is responsible for elastic collisions. To do so, equate the first term on the RHS of (\ref{eqn:Qe-t}) with $T\rightarrow0$ to the RHS of (\ref{eqn:Qe-n}):
%
\begin{equation}
  \frac{3 \beta_k^{\rm n} k_{\rm B} |C_{\rm e}| \rho_{\rm e} \rho_k  T_{\rm e}}{m_{\rm e} m_k^2 (\mu_{\rm e}N)_k}
 =
  |C_{\rm e}| N_{\rm e} N_k (\mu_{\rm e}  N)_k  (E^\star_k)^2_{\rm tr}
\end{equation}
%
Then, isolate $(E^\star_k)^2_{\rm tr}$:
%
\begin{equation}
\frameeqn{
(E^\star_k)^2_{\rm tr}=\frac{3  k_{\rm B} T_{\rm e}  }{ m_k (\mu_{\rm e}N)_k^2}
}
\end{equation}
%
Proceed similarly for the electron-vibration collisions:
%
\begin{equation}
\frac{3}{2} N_{\rm e} N_k k_{\rm B} T_{\rm e} \frac{\nu_{\rm ev}}{N_k}
 =
  |C_{\rm e}| N_{\rm e} N_k (\mu_{\rm e}  N)_k  (E^\star_k)^2_{\rm vib}
\end{equation}
%
Then, isolate $(E^\star_k)^2_{\rm vib}$:
%
\begin{equation}
\frameeqn{
    (E^\star_k)^2_{\rm vib}
=
\frac{3 k_{\rm B} T_{\rm e} }{2 |C_{\rm e}|  (\mu_{\rm e}  N)_k}  \frac{\nu_{\rm ev}}{N_k}
}
\end{equation}
%
where $\frac{\nu_{\rm ev}}{N_k}$ is function solely of $T_{\rm e}$.


Then, we can subtract $(E^\star_k)^2_{\rm tr}$ and $(E^\star_k)^2_{\rm vib}$ from $(E^\star_k)^2$ before adding $Q_{\rm e-n}$ to $Q_{\rm e-t}$:
%
\begin{equation}
\frameeqn{
Q_{\rm e}
= 
  \sum_k \beta_k^{\rm n} |C_{\rm e}| N_{\rm e} N_k (\mu_{\rm e}  N)_k  \left( (E^\star_k)^2 -(E^\star_k)^2_{\rm tr} - (E^\star_k)^2_{\rm vib}\right)
+  \sum_k  \frac{3 \beta_k^{\rm n} k_{\rm B} |C_{\rm e}| N_{\rm e} N_k  (T_{\rm e}-T)}{ m_k (\mu_{\rm e}N)_k}        
 + \sum_k \beta_k^{\rm i}  N_{\rm e} N_k  (T_{\rm e}-T )  \frac{6 k_{\rm B} C_i^2 C_e^2 \ln \Lambda}{ \pi^3 \epsilon_0^2 m_{\rm e} m_k \overline{q_{\rm e}}^3}  
}
\label{eqn:Qe}
\end{equation}
%
The latter includes all electron energy losses to the ions and the neutrals either due to elastic or inelastic processes. It also includes electron energy gains due to elastic processes. However, it does not include electron energy gains due to inelastic processes when the electron temperature is less than the gas temperature. Such are, however, expected to be small and not well known. Also, they would not affect significantly the electron temperature except perhaps when the latter is much less than the gas temperature.
...
Q_{\rm e}
= 
&  \sum_k \beta_k^{\rm n} |C_{\rm e}| N_{\rm e} N_k (\mu_{\rm e}  N)_k  \left( (E^\star_k)^2 -(E^\star_k)^2_{\rm tr}\right)
+  \sum_k  \frac{3 \beta_k^{\rm n} k_{\rm B} |C_{\rm e}| \rho_{\rm e} \rho_k  (T_{\rm e}-T)}{m_{\rm e} m_k^2 (\mu_{\rm e}N)_k}   
\nonumber\\     
...
\end{align}
%
The first term on the RHS needs to be rewritten slightly differently to become numerically stable:
%
\begin{align}
Q_{\rm e}
= 
&  \sum_k \frac{\beta_k^{\rm n} |C_{\rm e}| \rho_{\rm e} \rho_k (\mu_{\rm e}  N)_k (T_{\rm e}-T_{\rm ref})}{m_{\rm e} m_k T_{\rm e}} \max\left( (E^\star_k)^2 -(E^\star_k)^2_{\rm tr},~0\right)
+  \sum_k  \frac{3 \beta_k^{\rm n} k_{\rm B} |C_{\rm e}| \rho_{\rm e} \rho_k  (T_{\rm e}-T)}{m_{\rm e} m_k^2 (\mu_{\rm e}N)_k}   
\nonumber\\     
 &+ \sum_k \beta_k^{\rm i}  \rho_{\rm e} \rho_k  (T_{\rm e}^{-0.5}-T T_{\rm e}^{-1.5}) \left(\frac{\pi m_{\rm e}}{8 k_{\rm B}}\right)^{1.5}  \frac{6 k_{\rm B} C_i^2 C_e^2 \ln \Lambda}{ \pi^3 \epsilon_0^2 m_{\rm e}^2 m_k^2}  
\end{align}
%
with $T_{\rm ref}$ a small number less than the smallest temperature in the domain (typically set to 10~K).
...
= -\sum_{r=1}^\ncs \sum_{j=1}^3 \frac{\partial q^{k}_j}{\partial x_j}
- \sum_{r=1}^\ncs Q_{r} \alb\mfd
...
\begin{equation}
\begin{array}{l}\mfd
 \frac{\partial }{\partial t}\left(\sum_{k=1}^\ns \rho_k (e_k+h_k^\circ)+\frac{1}{2}\rho_{\rm n}|\vec{V}|^2 \right) 
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(\sum_{k=1}^\ns \rho_k \vec{V}^k_j (h_k+h_k^\circ)+\frac{1}{2}\rho_{\rm n}\vec{V}_j|\vec{V}|^2 \right)
= \alb\mfd
-\sum_{r=1}^\ncs \sum_{i=1}^3 \frac{\partial q^{k}_i}{\partial x_i}
-\sum_{i=1}^{3}\frac{\partial q_i^{\rm n}}{\partial x_i}
+\sum_{i=1}^3 \sum_{j=1}^3  \frac{\partial }{\partial x_j} \tau_{ji} \vec{V}_i
+\frac{1}{2}|\vec{V}|^2 \sum_{k=1}^\nns W_k
+ \vec{E}\cdot\vec{J}
-  Q_{\rm v}
...
\begin{equation}
\begin{array}{l}\mfd
 \frac{\partial }{\partial t}\left(\rho_{\rm N_2} \ev+\sum_{k=1}^\ns \rho_k (e_k+h_k^\circ)+\frac{1}{2}\rho_{\rm n}|\vec{V}|^2 \right)\alb\mfd 
...
\frac{\partial }{\partial t} \left(\frac{1}{2}\rho_{\rm n}|\vec{V}|^2\right)
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(
\frac{1}{2}\rho_{\rm n}\vec{V}_j|\vec{V}|^2
\right)
-\frac{1}{2}|\vec{V}|^2 \sum_{k=1}^\nns W_k\alb\mfd
\approx
\frac{\partial }{\partial t} \left(\frac{1}{2}\rho |\vec{V}|^2\right)
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(
\frac{1}{2}\rho \vec{V}_j|\vec{V}|^2
\right)
\end{array}
\label{eqn:etotal2}
\end{equation}
...
\frameeqn{
\begin{array}{l}\mfd
 \frac{\partial }{\partial t}\left(\rho_{\rm N_2} \ev+\sum_{k=1}^\ns \rho_k (e_k+h_k^\circ)+\frac{1}{2}\rho|\vec{V}|^2 \right) \alb\mfd
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(\rho_{\rm N_2} \vec{V}_j^{\rm N_2} \ev + \sum_{k=1}^\ns \rho_k \vec{V}^k_j (h_k+h_k^\circ)+\frac{1}{2}\rho \vec{V}_j|\vec{V}|^2 \right)
 \alb\mfd
=
-\sum_{r=1}^\ncs \sum_{i=1}^3 \frac{\partial q^{k}_i}{\partial x_i}
-\sum_{i=1}^{3}\frac{\partial q_i^{\rm n}}{\partial x_i}
...
\begin{array}{l}\mfd
 \frac{\partial }{\partial t}\left(\rho_{\rm N_2} \ev+\sum_{k=1}^\ns \rho_k (e_k+h_k^\circ)+\frac{1}{2}\rho|\vec{V}|^2 \right) \alb\mfd
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(\rho_{\rm N_2} \vec{V}_j \ev + \sum_{k=1}^\ns \rho_k \vec{V}_j (h_k+h_k^\circ)+\frac{1}{2}\rho \vec{V}_j|\vec{V}|^2 \right)\alb\mfd
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(\rho_{\rm N_2} (\vec{V}_j^{\rm N_2}-\vec{V}_j) \ev + \sum_{k=1}^\ns \rho_k (\vec{V}^k_j-\vec{V}_j) (h_k+h_k^\circ) \right)
 \alb\mfd
=
-\sum_{r=1}^\ncs \sum_{i=1}^3 \frac{\partial q^{k}_i}{\partial x_i}
-\sum_{i=1}^{3}\frac{\partial q_i^{\rm n}}{\partial x_i}
...
%
\begin{equation}
e_{\rm t} \equiv w_{\rm N_2} \ev + \sum_{k=1}^\ns w_k (e_k + h_k^\circ) +\frac{1}{2} |\vec{V}|^2
\end{equation}
%
...
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \vec{V}_j \left(\rho  e_{\rm t} +  P \right)
- \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(
   \nu_{\rm N_2} \ev\frac{\partial w_{\rm N_2}}{\partial x_j} + \sum_{k=1}^\ns \beta_k^{\rm n} \nu_k (h_k+h_k^\circ)\frac{\partial w_k}{\partial x_j} 
\right)
 \alb\mfd
+ \sum_{j=1}^3 \sum_{k=1}^\ns  \frac{\partial }{\partial x_j} \left(
  \beta_k^{\rm c} w_k (\vec{V}^k_j-\vec{V}_j) \frac{(h_k+h_k^\circ)}{e_{\rm t}} \rho e_{\rm t} 
\right)\alb\mfd
=
-\sum_{r=1}^\ncs \sum_{i=1}^3 \frac{\partial q^{k}_i}{\partial x_i}
-\sum_{i=1}^{3}\frac{\partial q_i^{\rm n}}{\partial x_i}
...
+ \sum_{j=1}^3 \sum_{k=1}^\ns  \frac{\partial }{\partial x_j} \left(
  \beta_k^{\rm c} w_k (\vec{V}^k_j-\vec{V}_j) \frac{(h_k+h_k^\circ)}{e_{\rm t}} \rho e_{\rm t} 
\right)
-\sum_{i=1}^{3}\frac{\partial }{\partial x_i}\left(\kappa_{\rm e} \frac{\partial T_{\rm e}}{\partial x_i}+(\kappa_{\rm n}+\kappa_{\rm i}) \frac{\partial T}{\partial x_i} 
...
\end{equation}
%  
where $\kappa_{\rm i}$ is the sum of the ion thermal conductivities:
%
\begin{equation}
\kappa_{\rm i}=\sum_k \beta_k^{\rm i} \kappa_k
\end{equation}
%
...
\begin{equation}
 Q_{\rm J} =   \sum_{r=1}^\ncs \left( C_r N_r  \vec{E}   \right)\cdot \vec{V}^r 
+  \sum_{r=1}^\ncs \vec{\nabla} P_r \cdot \left(\vec{V}-\vec{V}^r\right) 
- \left(\rho_{\rm c} \vec{E} + \vec{J}\times\vec{B} \right)\cdot \vec{V}
\end{equation}
%
...
+ \sum_{j=1}^3 \frac{\partial }{\partial x_j} \visc^\star  \left( \frac{\partial \vec{V}_i}{\partial x_j} + \frac{\partial \vec{V}_j}{\partial x_i} - \frac{2}{3} \delta_{ij} \sum_{k=1}^3 \frac{\partial \vec{V}_k}{\partial x_k}  \right)
+ \rho_{\rm c} \vec{E}_i +\left(\vec{J} \times \vec{B}\right)_i
\end{array}
}
...
+ \sum_{j=1}^3  \frac{\partial }{\partial x_j} \vec{V}_j \left(\rho  e_{\rm t}^\star +  P^\star \right)
- \sum_{j=1}^3  \frac{\partial }{\partial x_j} \left(
   \nu_{\rm N_2}^\star \ev\frac{\partial w_{\rm N_2}}{\partial x_j} + \sum_{k=1}^\ns \beta_k^{\rm n} \nu_k^\star (h_k+h_k^\circ)\frac{\partial w_k}{\partial x_j} 
\right)
 \alb\mfd
+ \sum_{j=1}^3 \sum_{k=1}^\ns  \frac{\partial }{\partial x_j} \left(
  \beta_k^{\rm c} w_k (\vec{V}^k_j-\vec{V}_j) \frac{(h_k+h_k^\circ)}{e_{\rm t}^\star} \rho e_{\rm t}^\star 
\right)\alb\mfd
-\sum_{i=1}^{3}\frac{\partial }{\partial x_i}\left((\kappa_{\rm i}+\kappa_{\rm n}^\star) \frac{\partial T}{\partial x_i} + \kappa_{\rm e} \frac{\partial T_{\rm e}}{\partial x_i} + \kappa_{\rm v}^\star \frac{\partial T_{\rm v}}{\partial x_i}
+ \visc_{k}^\star \frac{\partial k}{\partial x_i}\right)
\alb\mfd
=
 \sum_{i=1}^3 \sum_{j=1}^3  \frac{\partial }{\partial x_j}  \vec{V}_i \visc^\star  \left( \frac{\partial \vec{V}_i}{\partial x_j} + \frac{\partial \vec{V}_j}{\partial x_i} - \frac{2}{3} \delta_{ij} \sum_{k=1}^3 \frac{\partial \vec{V}_k}{\partial x_k} \right)
+ \vec{E}\cdot\vec{J}
+ Q_{\rm b}
\end{array}
}
...
\frameeqn{
   ...
   % convection term
  + \sum_{j=1}^{3}\frac{\partial }{\partial x_j} \rho \vec{V}_j k
...
\begin{equation}
  Q_k= \mfd\sum_{i=1}^3 \sum_{j=1}^{3}
         \left[ \visc_{\rm t} \left(
           \frac{\partial {\vec{V}_i}}{\partial x_j}
           +\frac{\partial {\vec{V}_j}}{\partial x_i}
           -\frac{2}{3} \delta_{ij} \sum_{k=1}^3 \frac{\partial {\vec{V}_k}}{\partial x_k}
            \right)
...
\mfd \frac{\rho k}{{\rm max}\left(\omega,~\mfd\frac{35}{12}\sqrt{\frac{1}{2}\sum_{i=1}^3\sum_{j=1}^3\left(\frac{\partial \vec{V}_i}{\partial x_j}+\frac{\partial \vec{V}_j}{\partial x_i}-\frac{2}{3}\delta_{ij}\sum_{k=1}^{3} \frac{\partial \vec{V}_k}{\partial x_k}\right)^2}\right)}
...
\frac{\partial \vec{V}_i}{\partial x_j}-\frac{\partial \vec{V}_j}{\partial x_i}
\right)^2
} %sqrt
\right)}
...
\frameeqn{
 \begin{array}{r}
        ...
        +\mfd\sum_{j=1}^{3}
          \frac{\partial }{\partial x_j} \rho \vec{V}_j \omega
...
\left(\frac{\partial \vec{V}_i}{\partial x_j}-\frac{\partial \vec{V}_j}{\partial x_i} \right)
\left(\frac{\partial \vec{V}_j}{\partial x_k}-\frac{\partial \vec{V}_k}{\partial x_j} \right)
\left(\frac{\partial \vec{V}_k}{\partial x_i}+\frac{\partial \vec{V}_i}{\partial x_k} 
     - \delta_{ki}\sum_{m=1}^3\frac{\partial \vec{V}_m}{\partial x_m}  \right)
\right|
\end{equation}
%
...
F_i = \left[ \begin{array}{c}
    ...
    \vdots\\
    \rho_\ns \vec{V}_i \alb
    \rho\vec{V}_i \vec{V}_1 + \delta_{i1} P^\star \\
    \vdots \\
    \rho\vec{V}_i \vec{V}_3  + \delta_{i3} P^\star \alb
    \rho \vec{V}_i e_{\rm t}^\star + \vec{V}_i P^\star \alb
    \rho \vec{V}_i k \alb
    \rho \vec{V}_i \omega \alb
    \rho_{\rm N_2} \vec{V}_i e_{\rm v} \alb
    \rho_{\rm e} \vec{V}_i h_{\rm e} \alb
\end{array}\right]
\end{equation}
%
%
...
W_{\rm e} e_{\rm e} +   C_{\rm e} N_{\rm e} \vec{E} \cdot \vec{V}^{\rm e}  - Q_{\rm e} \alb
\end{array}
\right]
\end{equation}
%
...
\alpha_{kr}\Delta \vec{V}_i^r+ (\alpha_{kr}-\delta_{kr})\vec{V}_i+\delta_{kr}(s_k-\beta_k^{\rm g}+ \beta^{\rm a}_k) \mu_k  \vec{E}_i\right) &\textrm{if $k\le \ns$ and $r \le \ns$} \alb
        \frac{1}{e_{\rm t}^\star} \sum_{m=1}^\ns \beta_m^{\rm c} w_m (h_m+h_m^\circ) \left(\vec{V}_i^{m}-\vec{V}_i\right)  &\textrm{if $k=r=\ns+4$ } \alb
        \frac{5}{3}(\vec{V}_i^{\rm e}-\vec{V}_i) &\textrm{if $k=r=\ns+8$}
             \end{array}  \right.
\end{equation}
%
...
\begin{equation}
  [D_i]_{k,k}=\left\{\begin{array}{ll}
        \beta_k^{\rm c} \left(\sum_{r=1}^\ns \frac{\delta_{rk}\sigma+\beta_k^{\rm a} C_r  \mu_k N_r}{\sigma} \left(\Delta \vec{V}_i^r +(1-\delta_{kr})\vec{V}_i\right)  +(s_k-\beta_k^{\rm g}+ \beta^{\rm a}_k) \mu_k  \vec{E}_i\right) &\textrm{if $k\le \ns$ } \alb
        \frac{1}{e_{\rm t}^\star} \sum_{m=1}^\ns \beta_m^{\rm c} w_m (h_m+h_m^\circ) \left(\vec{V}_i^{m}-\vec{V}_i\right)  &\textrm{if $k=\ns+4$ } \alb
        \frac{5}{3}(\vec{V}_i^{\rm e}-\vec{V}_i) &\textrm{if $k=\ns+8$}
             \end{array}  \right.
\end{equation}
%
...
\begin{equation}
  [K_{ij}]_{k,r}=\left\{\begin{array}{ll}
         ...
         \sum_{m=1}^3  [\psi_{ij}]_{m,r-\ns}\visc^\star \vec{V}_m & \textrm{if $k=\ns+4$ and $\ns < r \leq \ns+3$} \alb

         \delta_{ij} (\kappa_{\rm n}^\star+\kappa_{\rm i}) & \textrm{if $k=\ns+4$ and $r=\ns +4$} \alb
