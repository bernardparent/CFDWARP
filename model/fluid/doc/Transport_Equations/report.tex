...
For the charged species that are not injected at the surface, the net mass flux must be zero. Because the charged species transport is obtained from the drift-diffusion model, this yields the following condition:
%
\begin{equation}
 \vec{V}^k_\chi=0
\end{equation}
%
with $\vec{V}^k$ the velocity of the $k$th charged species including drift and diffusion as outlined in Eq.\ (\ref{eqn:V}):
%
\begin{equation}
  \vec{V}^{k}_i = \vec{V}^{\rm n}_i+\sum_{j=1}^3 s_k \wtilde{\mu}^k_{ij}  \vec{E}_j^{\rm n}
             - \sum_{j=1}^3  \frac{\wtilde{\mu}^{k}_{ij}}{|C_k| N_k} \frac{\partial P_k}{\partial x_j}
\end{equation}
%
Consider no magnetic field for simplicity and set the $\chi$ component to zero:
%
\begin{equation}
  0 = \vec{V}^{\rm n}_\chi+ s_k \mu_k  \vec{E}_\chi^{\rm n}
             - \frac{\mu_k}{|C_k| N_k} \frac{\partial P_k}{\partial \chi}
\end{equation}
%
