...
\end{equation}
%
And the boundary condition for the O$_2$ mass fraction becomes:
%
\begin{equation}
 \frac{\partial w_{\rm O_2}}{\partial \chi}  = -\frac{{\cal M}_{\rm O}}{{\cal M}_{\rm O_2}} \frac{\partial w_{\rm O}}{\partial \chi}
\end{equation}
%
with ${\cal M}_k$ the molecular weight of species $k$.
...
\section{Surface Injection Boundary Conditions}

Consider a surface through which some species $i$ is injected into the flow. Say that the total amount of gas injected per surface area is equal to $\dot{m}^{\rm in}$. The bulk gas velocity component perpendicular to the surface must thus be equal to:
%
\begin{equation}
\frameeqn{
  \vec{V}_\chi = \frac{\dot{m}^{\rm in}}{\rho}
 }
\end{equation}
%
...
For species that have no mass injection at the surface, the net mass flow must be zero. Thus, the sum of the convective and diffusive mass flow must cancel out:
%
\begin{equation}
  \rho_k \vec{V}_\chi - \nu_k \frac{\partial c_k}{\partial \chi} =0
\end{equation}
%
Isolate the partial derivative:
%
\begin{equation}
 \frac{\partial c_k}{\partial \chi}= \frac{\rho_k \vec{V}_\chi}{\nu_k} 
\end{equation}
%
But the velocity perpendicular to the wall is related to $\dot{m}^{\rm in}$:
%
\begin{equation}
 \frac{\partial c_k}{\partial \chi}= \frac{\rho_k \dot{m}^{\rm in}}{\rho \nu_k} 
\end{equation}
%
...
\begin{equation}
\frameeqn{
 \frac{\partial c_k}{\partial \chi}= \frac{c_k \dot{m}^{\rm in}}{\nu_k} ~~~~~\textrm{for~non-injected~species}
}
\end{equation}
%

As for the species that are injected at the surface, the net mass flow in the domain must be equal to $\dot{m}_{\rm in}^k$:
%
\begin{equation}
  \rho_k \vec{V}_\chi - \nu_k \frac{\partial c_k}{\partial \chi} =\dot{m}^{\rm in}_k
\end{equation}
%
But $\vec{V}_\chi$ is equal to $\dot{m}^{\rm in}/\rho$ and $\rho_k/\rho=c_k$:
%
\begin{equation}
  c_k \dot{m}_{\rm in} - \nu_k \frac{\partial c_k}{\partial \chi} =\dot{m}^{\rm in}_k
\end{equation}
...
\begin{equation}
\frameeqn{
\frac{\partial c_k}{\partial \chi}=  \frac{c_k \dot{m}^{\rm in} - \dot{m}^{\rm in}_k}{ \nu_k }
~~~~~\textrm{for~injected~species}
}
\end{equation}
%
In the latter, the tal mass flow rate injected per unit area corresponds to the sum of the injected species mass flow rates per unit area:
%
\begin{equation}
  \dot{m}^{\rm in} = \sum_k \dot{m}^{\rm in}_k
\end{equation}
%
