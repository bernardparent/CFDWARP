...
void find_MUstar_from_jacvars(jacvars_t jacvars,  metrics_t metrics, flux_t MUstar){
  ...
  MUstar[fluxev]=jacvars.ev*Pstar*dPdrhoetstar/(2.0*a2*a2);
#endif
  ...
  MUstar[fluxev]=jacvars.ev*Pstar*dPdrhoetstar/(3.0*a2*a2);
#endif 
...
void find_jacvars_at_interface_Parent_average(jacvars_t jacvarsL, jacvars_t jacvarsR, gl_t *gl, long theta, metrics_t metrics, jacvars_t *jacvars){
  ...
  long dim,spec;
  double etstar,rho,q2;
  spec_t dedrhok;
  dim_t VL,VR,V;
  double eL,eR,kL,kR,psiL,psiR,TL,TR,TvL,TvR,evL,evR;
  double Xmag2,Vstar,Mtheta;

  find_primvars_from_jacvars(jacvarsL, rhokL, VL, &TL, &eL, &kL, &psiL, &TvL, &evL);
  find_primvars_from_jacvars(jacvarsR, rhokR, VR, &TR, &eR, &kR, &psiR, &TvR, &evR);
...
//    V[dim]=0.5*VL[dim]+0.5*VR[dim];
  ...
  jacvars->T=_f_Parent_average(TL,TR,Mtheta);
  jacvars->k=_f_Parent_average(kL,kR,Mtheta);
  jacvars->psi=_f_Parent_average(psiL,psiR,Mtheta);
  jacvars->ev=_f_Parent_average(evL,evR,Mtheta);
  jacvars->P=_P_from_w_rho_T(jacvars->w,rho,jacvars->T);
  etstar = _e_from_w_T(jacvars->w,jacvars->T)+0.5e0*q2 + jacvars->k + jacvars->w[specN2]*jacvars->ev;
  jacvars->htstar=etstar+jacvars->P/rho+2.0/3.0*jacvars->k;
  jacvars->dPdrhoetstar=1.0e0/(rho*_de_dP_at_constant_rho(rhok,jacvars->T));
  find_de_drhok_at_constant_P(rhok,jacvars->T,dedrhok);
  for (spec=0; spec<ns; spec++){
      jacvars->dPdrhok[spec]=-(jacvars->dPdrhoetstar)*(etstar-q2-jacvars->k-jacvars->w[specN2]*jacvars->ev
                                 +dedrhok[spec]*rho);
  }
