...
//            musclvarsR[flux]=_f_TVD5(musclvarsp5h[flux],musclvarsp3h[flux],musclvarsp1h[flux],musclvarsm1h[flux],musclvarsm3h[flux]);
  ...
  switch (gl->cycle.resconv.FLUX){
    case FLUX_FDS:
#ifdef _RESTIME_CDF  
      find_Fstar_interface_FDSplus_muscl(np, gl,  l, _al(gl,l,theta,+1), theta, musclvarsL, musclvarsR, metrics, 0, EIGENVALUES_NOT_ENFORCED_POSITIVE,  EIGENVALCOND, gl->cycle.resconv.AVERAGING, Fint, lambdaminusp1h, lambdaplusm1h);
#else
      find_Fstar_interface_FDS_muscl(gl, theta, musclvarsL, musclvarsR, metrics, EIGENVALCOND, gl->cycle.resconv.AVERAGING, Fint);
#endif
    break;
    case FLUX_FVS:
#ifdef _RESTIME_CDF  
      find_Fstar_interface_FVSplus_muscl(np, gl,  l, _al(gl,l,theta,+1), theta, musclvarsL, musclvarsR, metrics, 0, EIGENVALUES_NOT_ENFORCED_POSITIVE, EIGENVALCOND, gl->cycle.resconv.AVERAGING, Fint, lambdaminusp1h, lambdaplusm1h);
#else

      find_Fstar_interface_FVS_muscl(gl, theta, musclvarsL, musclvarsR, metrics, EIGENVALCOND, Fint);
#endif
