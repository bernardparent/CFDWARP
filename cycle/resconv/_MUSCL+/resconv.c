...
void read_disc_resconv_actions(char *actionname, char **argum, SOAP_codex_t *codex){
  ...
  if (strcmp(actionname,_RESCONV_ACTIONNAME)==0) {
    ...
    if (gl->cycle.resconv.FLUX!=FLUX_FDSplus && gl->cycle.resconv.FLUX!=FLUX_FVSplus && gl->cycle.resconv.FLUX!=FLUX_FDSplusgen)
      SOAP_fatal_error(codex,"FLUX must be set to either FLUX_FDSplus or FLUX_FVSplus or FLUX_FDSplusgen.");
...
static void find_Fstar_interface(np_t *np, gl_t *gl, long l, long theta, flux_t musclvarsm5h, flux_t musclvarsm3h, flux_t musclvarsm1h, flux_t musclvarsp1h, flux_t musclvarsp3h, flux_t musclvarsp5h, flux_t Fint, sqmat_t lambdaminusp1h, sqmat_t lambdaplusm1h){
  ...
  switch (gl->cycle.resconv.FLUX){
    ...
    break;
    default:
      fatal_error("gl->cycle.resconv.FLUX must be set to either FLUX_FDSplus, FLUX_FVSplus, or FLUX_FDSplusgen in find_Fstar_interface().");
  }
