...
void convert_Fstar_interface_positivity_preserving(np_t *np, gl_t *gl, long lp0, long theta, metrics_t metrics, long numiter, int EIGENVALCOND, flux_t Fint, flux_t Fpositive, sqmat_t lambdaminus, sqmat_t lambdaplus){

  flux_t MUp0,MUp1,Up0,Up1,Vp1,Vp0;
  sqmat_t Mp0,Mp1,Minvp0,Minvp1;
  ...
  find_M_from_jacvars(jacvarsp0, metrics, Mp0);
  find_M_from_jacvars(jacvarsp1, metrics, Mp1);
  find_Minv_from_jacvars(jacvarsp0, metrics, Minvp0);
  find_Minv_from_jacvars(jacvarsp1, metrics, Minvp1);

  find_Ustar_from_jacvars(jacvarsp0,  metrics, Up0);
  find_MUstar_from_jacvars(jacvarsp0, metrics, MUp0);
  find_Ustar_from_jacvars(jacvarsp1,  metrics, Up1);
  ...
  for (cnt=0; cnt<numiter; cnt++){
    multiply_matrix_and_vector(Zplus,MUp1,fluxtmp2);
    multiply_matrix_and_vector(Minvp1,fluxtmp2,fluxtmp);
    multiply_matrix_and_vector(Mp0,fluxtmp,fluxtmp2);
    multiply_matrix_and_vector(Yminus,MUp0,fluxtmp3);
    multiply_matrix_and_vector(Minvp0,fluxtmp3,fluxtmp);
    multiply_matrix_and_vector(Mp1,fluxtmp,fluxtmp3);
    for (flux=0; flux<nf; flux++) {
      Yminus[flux][flux]=min(0.0, Yplus[flux][flux]+fluxtmp2[flux]/notzero(MUp0[flux],1e-99));
      Zplus[flux][flux]=max(0.0, Zminus[flux][flux]+fluxtmp3[flux]/notzero(MUp1[flux],1e-99));
    }
    for (flux=0; flux<nf; flux++) {
      Yplus[flux][flux]=max(0.0, Yplus[flux][flux]+fluxtmp2[flux]/notzero(MUp0[flux],1e-99));
      Zminus[flux][flux]=min(0.0, Zminus[flux][flux]+fluxtmp3[flux]/notzero(MUp1[flux],1e-99));
    }   
  }

  for (flux=0; flux<nf; flux++) {
    lambdaplus[flux][flux]=Yplus[flux][flux]+Zplus[flux][flux];///MUp0[flux]*MUp1[flux];
  ...
  multiply_diagonal_matrix_and_vector(lambdaminus,MUp1,fluxtmp2);
  multiply_matrix_and_vector(Minvp1,fluxtmp2,Fp1);
